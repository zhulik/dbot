...
module StartCommand
  def start(*)
    ...
    respond_message text: t('.hi', name: user_greeting(from))
  end

  protected

  def start_with_existing_user
    return respond_message text: t('.already_started') if current_user.active?

    current_user.update_attributes(active: true)
    respond_message text: t('.reactivated')
  end
end
